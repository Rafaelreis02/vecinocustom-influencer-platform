...
export default function MessagesPage() {
  ...
  const filteredEmails = emails.filter((email) => {
    ...
    // Apply status filter
    if (statusFilter !== 'all' && email.influencer?.status !== statusFilter) {
      return false;
    }
      ...
      <div className={`fixed md:static inset-y-0 left-0 z-40 w-64 border-r border-gray-200 bg-white flex flex-col transition-transform duration-300 ${
        ...
        <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
          ...
          <div className="pt-4 mt-4 border-t border-gray-200">
            <p className="px-4 text-xs font-semibold text-gray-500 uppercase mb-2">Por Estado</p>
            ...
            {getWorkflowStatuses().map((status) => {
              ...
              return (
                <button
                  key={status}
                  onClick={() => setStatusFilter(status as StatusFilterType)}
                  className={`w-full flex items-center gap-3 px-4 py-2 rounded-lg transition ${
                    statusFilter === status
                      ? 'bg-blue-100 text-blue-700 font-medium'
                      : 'text-gray-700 hover:bg-gray-100'
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <StatusDropdown
                        ...
                        currentStatus={fullInfluencer.status}
                        onStatusChange={() => fetchFullInfluencer(fullInfluencer.id)}
                        size="sm"
                      />
                      {fullInfluencer.tier && (
